...
import { getSkillCategory, categorizeKeyword } from './skillDictionaries';
...
function groupSkillsBySubCategory(keywords) {
  ...
  // 1. Group by technical sub-categories dynamically
  for (const kw of keywords) {
    const category = getSkillCategory(kw);
    if (category) {
      if (!groups[category]) {
        groups[category] = [];
      }
      groups[category].push(kw);
      used.add(kw);
    }
  }

  // 2. Group by other skill types (Soft Skills, Certifications)
  for (const kw of keywords) {
    if (used.has(kw)) continue;
    const cat = categorizeKeyword(kw);
    let label = null;
    if (cat === 'soft') label = 'Soft Skills';
    else if (cat === 'certification') label = 'Certifications';
  ...
  // 3. Anything remaining goes to "Other"
